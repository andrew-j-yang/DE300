...
def read_data(spark: SparkSession) -> DataFrame:
    ...
    titanic_data = spark.read.csv("s3://andrewawsbucket/sample.csv", header = True, inferSchema = True)
    return titanic_data
...
def pipeline(data: DataFrame):
    ...
    square_features = SquareFeatures(inputCols=imputed_columns_numeric, outputCols=squared_columns)
...
# After including the SquaredFeature transformer, and the 'numTrees' hyperparameter with values of [10, 20, 30], I saw that the AUC-ROC 
# value and the Selected Max Tree Depth was unchanged.
#
# The squared features likely did not add significant new information due to the original features being already well represented 
# in the model.
#
# The inclusion of the 'numTrees' hyperparameter in the cross-validation did not change the AUC-ROC value or 'Selected Max Tree Depth' 
# likely due to the 'maxDepth' hyperparameter having a more noticeable effect on the model's complexity and ability to learn from the data, 
# while 'numTrees' solely influenced the stability and variance reduction of the model. 
